...
#     Create a new directory to store the diff files
    ...
    if target == 'table':
        condition_table = table(cursor1, cursor2, dbname1, dbname2, host1, host2, port1, port2, target, ignore_schema,
                                choose_schema, ignore_table, choose_table)
        ...
        condition_function = function_view(cursor1, cursor2, dbname1, dbname2, host1, host2, port1, port2, target,
                                           ignore_schema, choose_schema, ignore_function, choose_function, ignore_view,
                                           choose_view)
        ...
        condition_view = function_view(cursor1, cursor2, dbname1, dbname2, host1, host2, port1, port2, target,
                                       ignore_schema, choose_schema, ignore_function, choose_function, ignore_view,
                                       choose_view)
...
class base(object):
    ...
    def database_structure_check(self):
        ...
        if self.choose_schema != '':
            for c in self.choose_schema:
                if (c,) not in s_schema:
                    print(' * [Host: %s][Port: %s][Database: %s]There is no \'schema\' %r' % (
                    self.host1, self.port1, self.dbname1, c))
                    self.choose_schema.remove(c)
                if (c,) not in c_schema:
                    print(' * [Host: %s][Port: %s][Database: %s]There is no \'schema\' %r' % (
                    self.host2, self.port2, self.dbname2, c))
                    self.choose_schema.remove(c)
            if self.choose_schema != '':
        ...
        if len(s_schema) == 0 and len(c_schema) != 0:
            print(
            ' * [Host: %s][Port: %s][Database: %s]There is no \'schema\'\n' % (self.host1, self.port1, self.dbname1))
        if len(s_schema) != 0 and len(c_schema) == 0:
            print(
        ...
        while s_schema_number < len(s_schema):
            while c_schema_number < len(c_schema):
                ...
                elif s_schema[s_schema_number] != c_schema[c_schema_number] and (c_schema_number + 1) >= len(
                        c_schema) and (s_schema_number + 1) < len(s_schema):
                    schema_count = schema_count + 1
                    print(' * [Host: %s][Port: %s][Database: %s]There is no schema %r\n' % (
        ...
        while c_schema_number < len(c_schema):
            while s_schema_number < len(s_schema):
                if c_schema[c_schema_number] == s_schema[s_schema_number] and (c_schema_number + 1) < len(c_schema):
                    ...
                    s_schema_number = s_schema_number + 1
                elif c_schema[c_schema_number] != s_schema[s_schema_number] and (s_schema_number + 1) >= len(
                        s_schema) and (c_schema_number + 1) < len(c_schema):
                    schema_count = schema_count + 1
                    print(' * [Host: %s][Port: %s][Database: %s]There is no schema %r\n' % (
    ...
    def schema_structure_check(target, cursor1, cursor2, dbname1, dbname2, host1, host2, port1, port2, schema_name):
        ...
        for s in schema_name:
            ...
            if target == 'table':
                ...
                cursor1.execute(
                    'SELECT routine_name FROM information_schema.routines WHERE routine_type=\'FUNCTION\' AND specific_schema=\'%s\';' %
                    s[0])
                s_schema_table = cursor1.fetchall()
            ...
            while s_schema_table_number < len(s_schema_table):
                while c_schema_table_number < len(c_schema_table):
                    if s_schema_table[s_schema_table_number] == c_schema_table[c_schema_table_number] and (
                        s_schema_table_number + 1) < len(s_schema_table):
                        s_schema_table_number = s_schema_table_number + 1
                        c_schema_table_number = 0
                    elif s_schema_table[s_schema_table_number] == c_schema_table[c_schema_table_number] and (
                        s_schema_table_number + 1) >= len(s_schema_table):
                        s_schema_table_number, c_schema_table_number = 0, 0
                        break
                    elif s_schema_table[s_schema_table_number] != c_schema_table[c_schema_table_number] and (
                        c_schema_table_number + 1) < len(c_schema_table):
                        c_schema_table_number = c_schema_table_number + 1
                    elif s_schema_table[s_schema_table_number] != c_schema_table[c_schema_table_number] and (
                        c_schema_table_number + 1) >= len(c_schema_table) and (s_schema_table_number + 1) < len(
                            s_schema_table):
            ...
            while c_schema_table_number < len(c_schema_table):
                while s_schema_table_number < len(s_schema_table):
                    if c_schema_table[c_schema_table_number] == s_schema_table[s_schema_table_number] and (
                        c_schema_table_number + 1) < len(c_schema_table):
                        c_schema_table_number = c_schema_table_number + 1
                        s_schema_table_number = 0
                    elif c_schema_table[c_schema_table_number] == s_schema_table[s_schema_table_number] and (
                        c_schema_table_number + 1) >= len(c_schema_table):
                        s_schema_table_number, c_schema_table_number = 0, 0
                        break
                    elif c_schema_table[c_schema_table_number] != s_schema_table[s_schema_table_number] and (
                        ...
                        s_schema_table_number + 1) >= len(s_schema_table) and (c_schema_table_number + 1) < len(
                            c_schema_table):
                        schema_table_count = schema_table_count + 1
                        print(' * [Host: %s][Port: %s][Database: %s]There is no %s %r in schema %r\n' % (
...
class table(base):
    def __init__(self, cursor1, cursor2, dbname1, dbname2, host1, host2, port1, port2, target, ignore_schema,
                 choose_schema, ignore_table, choose_table):
        self.cursor1 = cursor1
        self.cursor2 = cursor2
        self.dbname1 = dbname1
        ...
        if self.choose_schema == '':
            ...
            for s_s in S_schema:
                ...
                for s_t in S_schema_table:
                    if not self.table_schema_compare(s_s[0], s_t[0], self.cursor1, self.cursor2, self.host1, self.host2,
                                                     self.port1, self.port2, self.dbname1, self.dbname2):
                        count = count + 1
            ...
            for s_s in S_schema:
                ...
                for s_t in S_schema_table:
                    if not self.table_schema_compare(s_s, s_t[0], self.cursor1, self.cursor2, self.host1, self.host2,
                                                     self.port1, self.port2, self.dbname1, self.dbname2):
                        count = count + 1

                if count == 0:
                    print(
                    ' *** In schema %r, the schema of \'table\' in those two databases are completely matching\n' % s_s)
                else:
                    print(
                    ' *** In schema %r, the schema of \'table\' in those two databases are not completely matching\n' % s_s)

    @staticmethod
    def table_schema_compare(choose_schema, choose_table, cursor1, cursor2, host1, host2, port1, port2, dbname1,
                             dbname2):
    ...
    def table_content_check(self):
        ...
        if not self.table_schema_compare(self.choose_schema[0], self.choose_table, self.cursor1, self.cursor2,
                                         self.host1, self.host2, self.port1, self.port2, self.dbname1, self.dbname2):
    ...
    def table_content_compare(self):
        ...
        if len(s_data) != len(c_data):
            sign = sign + 1
            print('\n ** The number of records in table \'%s.%s\' are not matching' % (
            self.choose_schema[0], self.choose_table))

        if len(s_colnames) == 1 and s_colnames[0] == 'id':
            sign = sign + 1
            print('\n * [Host: %s][Port: %s][Database: %s]The table \'%s.%s\' only has one column (id)' % (
            self.host1, self.port1, self.dbname1, self.choose_schema[0], self.choose_table))
            return
        ...
        while x < len(s_data):
            while i < len(c_data):
                while s_count < len(s_colnames) and c_count < len(c_colnames):
                    if s_colnames[s_count] == c_colnames[c_count] and s_data[x][s_count] == c_data[i][c_count] and (
                        s_count + 1) < len(s_colnames):
                        s_count = s_count + 1
                        c_count = 1
                    elif s_colnames[s_count] == c_colnames[c_count] and s_data[x][s_count] == c_data[i][c_count] and (
                        s_count + 1) >= len(s_colnames):
                        break
                        ...
                        if (i + 1) < len(c_data):
                            ...
                            sign = sign + 1
                            print('\n * [Host: %s][Port: %s][Database: %s]There is no matching record: %r' % (
                            self.host2, self.port2, self.dbname2, s_data[x]))
                            break
                    else:
                        c_count = c_count + 1
                if s_colnames[s_count] == c_colnames[c_count] and s_data[x][s_count] == c_data[i][c_count] and (
                ...
                while s_count < len(s_colnames) and c_count < len(c_colnames):
                    if s_colnames[s_count] == c_colnames[c_count] and s_data[x][s_count] == c_data[i][c_count] and (
                        ...
                        if (x + 1) < len(s_data):
                            ...
                            sign = sign + 1
                            print('\n * [Host: %s][Port: %s][Database: %s]There is no matching record: %r' % (
                            self.host1, self.port1, self.dbname1, c_data[i]))
                            break
                    else:
                        s_count = s_count + 1
                if s_colnames[s_count] == c_colnames[c_count] and s_data[x][s_count] == c_data[i][c_count] and (
        ...
        if sign == 0:
            print('\n *** The table \'%s.%s\' in those two databases are totally matching' % (
            self.choose_schema[0], self.choose_table))
        else:
            print('\n *** The table \'%s.%s\' in those two databases are not totally matching' % (
            self.choose_schema[0], self.choose_table))


class function_view(base):
    def __init__(self, cursor1, cursor2, dbname1, dbname2, host1, host2, port1, port2, target, ignore_schema,
                 choose_schema, ignore_function, choose_function, ignore_view, choose_view):
        self.cursor1 = cursor1
        self.cursor2 = cursor2
        self.dbname1 = dbname1
        self.dbname2 = dbname2
        ...
        if self.choose_schema == '':
            ...
            for s_s in S_schema:
                ...
                for s_t in S_schema_table:
                    if self.target == 'function':
                        if not self.specified_check(self.target, self.cursor1, self.cursor2, self.dbname1, self.dbname2,
                                                    self.host1, self.host2, self.port1, self.port2, s_s[0],
                                                    choose_function=s_t[0]):
            ...
            for s_s in S_schema:
                ...
                for s_t in S_schema_table:
                    if self.target == 'function':
                        if not self.specified_check(self.target, self.cursor1, self.cursor2, self.dbname1, self.dbname2,
                                                    self.host1, self.host2, self.port1, self.port2, s_s,
                                                    choose_function=s_t[0]):
                            print(' * The %r \'%s.%s\' in those two databases are not matching\n' % (
    ...
    def specified_check(target, cursor1, cursor2, dbname1, dbname2, host1, host2, port1, port2, choose_schema,
        ...
        if choose_view != '':
            cursor1.execute('select definition from pg_views where viewname = \'%s\' and schemaname=\'%s\';' % (
            choose_view, choose_schema))
            specified1 = cursor1.fetchall()

            cursor2.execute('select definition from pg_views where viewname = \'%s\' and schemaname=\'%s\';' % (
            choose_view, choose_schema))
            specified2 = cursor2.fetchall()
    ...
    def specified_exist_check(self):
        ...
        if self.choose_function != '':
            self.cursor1.execute(
                'select exists(select 1 from information_schema.routines where specific_schema like \'%s\' and routine_name like \'%s\');' % (
            ...
            if db1[0] == True and db2[0] == True:
                if self.specified_check(self.target, self.cursor1, self.cursor2, self.dbname1, self.dbname2, self.host1,
                                        self.host2, self.port1, self.port2, self.choose_schema[0],
                                        choose_function=self.choose_function):
            ...
            self.cursor1.execute(
                'select exists(select 1 from pg_views where schemaname=\'%s\' and viewname=\'%s\');' % (
