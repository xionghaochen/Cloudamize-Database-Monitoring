...
def main(argv):
    ...
    if target=='table':
        condition_table=table(cursor1,cursor2,dbname1,dbname2,host1,host2,port1,port2,target,ignore_schema,choose_schema,choose_table)
        if choose_schema!='' and choose_table!='':
            condition_table.table_content_check()
        else:
            condition_table.database_schema_check()
    elif target=='function':
        condition_function=function_view(cursor1,cursor2,dbname1,dbname2,target,ignore_schema,choose_schema,choose_function,choose_view)
        if choose_schema!='' and choose_function!='':
            if condition_function.specified_check(cursor1,cursor2,dbname1,dbname2,choose_schema,choose_function=choose_function):
        ...
        condition_view=function_view(cursor1,cursor2,dbname1,dbname2,target,ignore_schema,choose_schema,choose_function,choose_view)
        if choose_schema!='' and choose_view!='':
            if condition_view.specified_check(cursor1,cursor2,dbname1,dbname2,choose_schema,choose_view=choose_view):
...
class base(object):
    ...
    def __init__(self,cursor1,cursor2,dbname1,dbname2,host1,host2,port1,port2,ignore_schema):
    ...
    def database_structure_check(self):
        ...
        self.cursor1.execute('select schema_name from information_schema.schemata where schema_name <> \'information_schema\' and schema_name !~ E\'^pg_\' and schema_name not like \'%s\';'%self.ignore_schema)
        s_schema=self.cursor1.fetchall()
        
        self.cursor2.execute('select schema_name from information_schema.schemata where schema_name <> \'information_schema\' and schema_name !~ E\'^pg_\' and schema_name not like \'%s\';'%self.ignore_schema)
        ...
        if len(s_schema)==0 and len(c_schema)!=0:
            print(' * [Host: %s][Port: %s]In databses %r, there is no \'schema\'\n'%(self.host1,self.port1,self.dbname1))
        if len(s_schema)!=0 and len(c_schema)==0:
            print(' * [Host: %s][Port: %s]In database %r, there is no \'schema\'\n'%(self.host2,self.port2,self.dbname2))
    ...
    #         elif ANOTHER PART
            ...
            if len(s_schema_table)==0 and len(c_schema_table)!=0:
                print(' * [Host: %s][Port: %s]In database %r, there is no %r in schema %r\n'%(host1,port1,dbname1,target,s[0]))
            if len(s_schema_table)!=0 and len(c_schema_table)==0:
...
class table(base):
    ...
    def __init__(self,cursor1,cursor2,dbname1,dbname2,host1,host2,port1,port2,target,ignore_schema,choose_schema,choose_table):
        self.cursor1=cursor1
        self.cursor2=cursor2
        self.dbname1=dbname1
        self.dbname2=dbname2
    ...
    def database_schema_check(self):
        ...
        if self.choose_schema=='':
            ...
            for s_t in S_schema_table:
                if not self.table_schema_compare(self.choose_schema,s_t[0],self.cursor1,self.cursor2,self.host1,self.host2,self.port1,self.port2,self.dbname1,self.dbname2):
                    count=count+1
                    print('count=',count)
    ...
    def specified_exist_check(self):
        self.cursor1.execute('SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = \'%s\' AND table_name = \'%s\');'%(self.choose_schema,self.choose_table))
        db1=self.cursor1.fetchone()
        
        self.cursor2.execute('SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = \'%s\' AND table_name = \'%s\');'%(self.choose_schema,self.choose_table))
        ...
        if  db1[0]==True and db2[0]==True:
            ...
            if db1[0]==True and db2[0]==False:
                print(' * The table \'%s.%s\' can not be found in database %r'%(self.choose_schema,self.choose_table,self.dbname2))
            elif db2[0]==True and db1[0]==False:
                print(' * The table \'%s.%s\' can not be found in database %r'%(self.choose_schema,self.choose_table,self.dbname1))
            elif db1[0]==False and db2[0]==False:
                print(' * The table \'%s.%s\' can not be found in those two databases'%(self.choose_schema,self.choose_table))
                ...
                while s_count<len(s_colnames) and c_count<len(c_colnames):
                    if s_colnames[s_count]==c_colnames[c_count] and s_data[x][s_count]==c_data[i][c_count] and (s_count+1)<len(s_colnames):
                        ...
                        if (i+1)<len(c_data):
                            ...
                            sign=sign+1
                            print('\n * [Host: %s][Port: %s]For database %r, there is no matching record: %r'%(self.host2,self.port2,self.dbname2,s_data[x]))
                            break
                    else:
                        c_count=c_count+1
                if s_colnames[s_count]==c_colnames[c_count] and s_data[x][s_count]==c_data[i][c_count] and (s_count+1)>=len(s_colnames):
                ...
                while s_count<len(s_colnames) and c_count<len(c_colnames):
                    if s_colnames[s_count]==c_colnames[c_count] and s_data[x][s_count]==c_data[i][c_count] and (c_count+1)<len(c_colnames):
                        ...
                        if (x+1)<len(s_data):
                            ...
                            sign=sign+1
                            print('\n * [Host: %s][Port: %s]For database %r, there is no matching record: %r'%(self.host1,self.port1,self.dbname1,c_data[i]))
                            break
                    else:
                        s_count=s_count+1
                if s_colnames[s_count]==c_colnames[c_count] and s_data[x][s_count]==c_data[i][c_count] and (c_count+1)>=len(c_colnames):
...
class function_view(base):
    ...
    def __init__(self,cursor1,cursor2,dbname1,dbname2,host1,host2,port1,port2,target,ignore_schema,choose_schema,choose_function,choose_view):
        self.cursor1=cursor1
        self.cursor2=cursor2
        self.dbname1=dbname1
        self.dbname2=dbname2
            ...
            for s_s in S_schema:
                ...
                for s_t in S_schema_table:
                    if self.target=='function':
                        if not self.specified_check(self.cursor1,self.cursor2,self.dbname1,self.dbname2,s_s[0],choose_function=s_t[0]):
                            print(' * The %r \'%s.%s\' in those two databases are not match\n'%(self.target,s_s[0],s_t[0]))
                            count=count+1
                    elif self.target=='view':
                        if not self.specified_check(self.cursor1,self.cursor2,self.dbname1,self.dbname2,s_s[0],choose_view=s_t[0]):
                            print(' * The %r \'%s.%s\' in those two databases are not match\n'%(self.target,s_s[0],s_t[0]))
                            count=count+1
    ...
    #     If self.choose_schema!='', then we check specified one schema.
            ...
            for s_t in S_schema_table:
                if self.target=='function':
                    if not self.specified_check(self.cursor1,self.cursor2,self.dbname1,self.dbname2,self.choose_schema,choose_function=s_t[0]):
                        print(' * The %r \'%s.%s\' in those two databases are not match\n'%(self.target,self.choose_schema,s_t[0]))
                        count=count+1
                elif self.target=='view':
                    if not self.specified_check(self.cursor1,self.cursor2,self.dbname1,self.dbname2,self.choose_schema,choose_view=s_t[0]):
                        print(' * The %r \'%s.%s\' in those two databases are not match\n'%(self.target,self.choose_schema,s_t[0]))
                        count=count+1
    ...
    def specified_exist_check(self):
        ...
        if self.choose_function!='':
            self.cursor1.execute('select exists(select 1 from information_schema.routines where specific_schema like \'%s\' and routine_name like \'%s\');'%(self.choose_schema,self.choose_function))
            db1=self.cursor1.fetchone()
            
            self.cursor2.execute('select exists(select 1 from information_schema.routines where specific_schema like \'%s\' and routine_name like \'%s\');'%(self.choose_schema,self.choose_function))
            db2=self.cursor2.fetchone()
    
            if  db1[0]==True and db2[0]==True:
                if self.specified_check(self.cursor1,self.cursor2,self.dbname1,self.dbname2,self.choose_schema,choose_function=self.choose_function):
                    print(' * In schema %r, the defination of function %r in those two databases are matching'%(self.choose_schema,self.choose_function))
                else:
                    print(' * In schema %r, the defination of function %r in those two databases are not matching'%(self.choose_schema,self.choose_function))
            else:
                if db1[0]==True and db2[0]==False:
                    print(' * The function \'%s.%s\' can not be found in database %r'%(self.choose_schema,self.choose_function,self.dbname2))
                elif db2[0]==True and db1[0]==False:
                    print(' * The function \'%s.%s\' can not be found in database %r'%(self.choose_schema,self.choose_function,self.dbname1))
                elif db1[0]==False and db2[0]==False:
                    print(' * The function \'%s.%s\' can not be found in those two databases'%(self.choose_schema,self.choose_function))
    
        elif self.choose_view!='':
            self.cursor1.execute('select exists(select 1 from pg_views where schemaname=\'%s\' and viewname=\'%s\');'%(self.choose_schema,self.choose_view))
            db1=self.cursor1.fetchone()
            
            self.cursor2.execute('select exists(select 1 from pg_views where schemaname=\'%s\' and viewname=\'%s\');'%(self.choose_schema,self.choose_view))
            db2=self.cursor2.fetchone()
    
            if  db1[0]==True and db2[0]==True:
                if self.specified_check(self.cursor1,self.cursor2,self.dbname1,self.dbname2,self.choose_schema,choose_function=self.choose_function):
                    print(' * In schema %r, the defination of view %r in those two databases are matching'%(self.choose_schema,self.choose_view))
                else:
                    print(' * In schema %r, the defination of view %r in those two databases are not matching'%(self.choose_schema,self.choose_view))
            else:
                if db1[0]==True and db2[0]==False:
                    print(' * The view \'%s.%s\' can not be found in database %r'%(self.choose_schema,self.choose_view,self.dbname2))
                elif db2[0]==True and db1[0]==False:
                    print(' * The view \'%s.%s\' can not be found in database %r'%(self.choose_schema,self.choose_view,self.dbname1))
                elif db1[0]==False and db2[0]==False:
                    print(' * The view \'%s.%s\' can not be found in those two databases'%(self.choose_schema,self.choose_view))
